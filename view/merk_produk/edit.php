...
<html>
<head>
	<title>Dashboard Merk Produk | Update</title>
	<meta charset="utf-8">
...
<body>
	...
	<div class="container" style="margin-top: 100px;">
		...
		<?php 
			...
			$query = mysqli_query($conn, "SELECT * FROM merk WHERE id_merk = $id_merk");
			$value = mysqli_fetch_array($query);
		 ?>
			<div class="form-group col-md-6 mx-auto border p-5 rounded">
				<h1 class="text-center">Ubah</h1>
				<hr>
				<input type="hidden" name="id_merk" value="<?= $value['id_merk'] ?>">
				<label for="nama_merk" class="font-weight-bold">Nama Merk</label>
				<input type="text" name="nama_merk" class="form-control" value="<?= $value['nama_merk'] ?>" required autocomplete="off" autofocus="on">
